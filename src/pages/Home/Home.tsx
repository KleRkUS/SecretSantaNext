...
const Home = () => {
    const [formState, setFormState] = useState<PlayersFormState>(initialFormState);
    ...
    const onSubmit = () => {
        ...
        fetch('/api/santa', {
            ...
            .then(async (res: Response) => {
                if (!res.ok) {
                    const parsed: { message: string } = (await res.json()) as {
                        message: string;
                    };
    ...
    return (
        <>
            <Flex alignItems="flex-start" justifyContent="flex-start" direction="column" padding="lg">
                <Typography weight="bold" size="lg" color="regular">
                    Add Players
                </Typography>

                <FormFields onUpdate={handleFormUpdate} currentPlayers={formState} />
                ...
                <Flex alignItems="center" justifyContent="flex-start" marginTop="md" marginBottom="lg">
                    <Typography weight="light">Allow players to be Santa for their own Santa.</Typography>
                    <input type="checkbox" onChange={toggleCrossPresentsState} checked={allowCrossPresents} />
                </Flex>
                        ...
                        <ul>
                            {Object.entries(results).map(([key, value]) => (
                                <li key={key}>
                                    ...
                                    {` ${formState.find(({ id }) => id === value)?.name ?? 'No match'}`}
                                </li>
                            ))}
                        </ul>
                    </Flex>

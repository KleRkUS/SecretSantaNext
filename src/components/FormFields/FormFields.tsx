...
export const FormFields = memo(({ onUpdate, currentPlayers }: FormFieldsProps) => {
    ...
    const deletePlayer = (id: number) => {
        if (currentPlayers.some(({ id: playerId }) => playerId === id)) {
            onUpdate(currentPlayers.filter(({ id: playerId }) => playerId !== id));
        }
    };
        ...
        <Flex alignItems="flex-start" direction="column">
            {currentPlayers.map(({ name, id, exclude }) => (
                <SPlayerRow justifyContent="flex-start" key={id}>
                    ...
                    <Button onClick={() => deletePlayer(id)} variant="regular" marginLeft="sm" marginRight="sm">
                        Delete player
                    </Button>
                    <Select
                        placeholder="Excluded players"
                        options={currentPlayers.filter((player) => player.id !== id) || []}
                        chosenIds={exclude}
                        updateOptions={updateExcluded(id)}
                    />
                    <Flex direction="row">
                        <Typography marginLeft="sm">Excluded players:</Typography>
                        {exclude.map((playerId: number) => (
                            <Typography weight="light" marginLeft="xs" key={playerId}>
                                {currentPlayers.map(({ id, name }) => (id === playerId ? name : null))}
                            </Typography>
                        ))}
                    </Flex>
                </SPlayerRow>
            ...
            <Flex justifyContent="flex-start">
                <InputText ref={newPlayerRef} placeholder="Add New Player" marginRight="sm" />
                <Button variant="regular" marginBottom="md" marginTop="md" marginLeft="sm" onClick={addPlayer}>
                    Add player
                </Button>
            </Flex>
        </Flex>
